...
// RouteObserver global — daftarkan di MaterialApp di main.dart jika belum ada
...
class _DashboardPresensiScreenState extends State<DashboardPresensiScreen>
  ...
  Map<String, dynamic> _rekap = {'hadir': 0, 'absen': 0, 'persentase': 0};
  ...
  void didChangeDependencies() {
    ...
    // Subscribe ke RouteObserver agar didPopNext terpanggil
  ...
  /// Dipanggil otomatis saat halaman ini kembali aktif (setelah pop dari PresensiScreen)
  @override
  void didPopNext() {
    _refreshJadwalStatus();
  }
  ...
  Future<void> _refreshJadwalStatus() async {
    ...
    final rekap = await JadwalService.instance.fetchRekapKehadiran();
    if (mounted) {
      setState(() {
        _rekap = rekap;
        _isRefreshing = false;
                                ...
                                children: [
                                  GestureDetector(
                                    onTap: () => _navigateToCamera(
                                      JadwalService.instance.allJadwal
                                          .firstWhere(
                                              (j) =>
                                                  j.mataKuliah ==
    ...
    if (result != null && result is Map) {
      if (result['status'] == 'success') {
        ...
        if (submitResult['success'] == true) {
          // Refresh rekap setelah presensi berhasil
          final rekap = await JadwalService.instance.fetchRekapKehadiran();
          setState(() {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                    submitResult['message'] ?? 'Presensi berhasil dicatat!'),
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                    submitResult['message'] ?? 'Gagal melakukan presensi.'),
  ...
  void _showPhotoDialog(String? imagePath) {
    ...
    if (imagePath.startsWith('/') ||
      ...
      final storageUrl =
          AuthService.baseUrl.replaceAll('/api', '/storage/$imagePath');
        ...
        loadingBuilder: (ctx, child, progress) {
          ...
          return const SizedBox(
            ...
            child: Center(
                child: CircularProgressIndicator(
              valueColor:
                  AlwaysStoppedAnimation<Color>(Color(0xFF14532D)),
            )),
          );
        },
        errorBuilder: (ctx, err, stack) => const Padding(
